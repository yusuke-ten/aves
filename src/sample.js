...
function Render() {
  ...
  var freq_max =
    c0_freq *
    Math.pow(freq_cent, controller.freq_min_cents + controller.freq_range_cents)
  ...
  for (
    var freq = freq_min, i = 0;
    freq < Math.min(freq_max, freq_nyquist);
    freq *= freq_step, ++i
  ) {
    ...
    x =
      ((i * 10) / controller.freq_range_cents) *
        (canvas.width - 2 * text_width) +
      text_width
    db = (20 * Math.log(fftMagSq / controller.ref_level)) / log10
    if (db < controller.db_min) db = controller.db_min
    y =
      (1 - (db - controller.db_min) / controller.db_range) *
      (canvas.height - 2 * text_height)
  ...
  for (
    var cents_rel = 0;
    cents_rel < controller.freq_range_cents + 10;
    cents_rel += 100
  ) {
    ...
    var x =
      (cents_rel / controller.freq_range_cents) *
        (canvas.width - 2 * text_width) +
      text_width
...
function StartProcessing(stream) {
  ...
  gui
    .add(controller, 'block_size', [128, 256, 512, 1024, 2048, 4096])
    .onChange(function(value) {
      UpdateController()
    })
  gui
    .add(controller, 'blocks_per_fft', [1, 2, 4, 8, 16, 32, 64])
    .onChange(function(value) {
      UpdateController()
    })
...
function Init() {
  ...
  try {
    ...
    console.log(
      'The Web Audio API is apparently not supported in this browser. ',
      e
    )
    message.innerHTML =
      'The Web Audio API is apparently not supported in this browser.'
  ...
  message.innerHTML =
    'To continue, please allow the application to access the audio capture device.'
